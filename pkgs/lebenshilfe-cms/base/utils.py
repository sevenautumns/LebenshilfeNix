...
def get_destatis_choices():
    file_path = Path(settings.BASE_DIR) / "base" / "data" / "destatis_nationalities_2024-08-01.json"
    ...
    raw_data = [
        entry for entry in data["daten"] 
        if entry[cols["ISO-2"]] and entry[cols["ISO-2"]] != "··"
    ]

    countries = [
        (e[cols["ISO-2"]], e[cols["Staatsname-kurz"]]) 
        for e in raw_data
    ]
    nationalities = [
        (e[cols["DESTATIS-Schluessel-Staatsangehoerigkeit"]], e[cols["Staatsangehoerigkeit"]]) 
        for e in raw_data
    ]
